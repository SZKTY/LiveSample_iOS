...
@MainActor
public struct AuthenticationCodeView: View {
    enum FocusStates {
        case code
    }
    ...
    @Dependency(\.viewBuildingClient.passwordView) var passwordView
    
    private let store: StoreOf<AuthenticationCode>
    
    public nonisolated init(store: StoreOf<AuthenticationCode>) {
        self.store = store
    }
    ...
    public var body: some View {
        WithViewStore(store, observe: { $0 }) { viewStore in
            ...
            .navigationDestination(
                store: store.scope(state: \.$destination.password,
